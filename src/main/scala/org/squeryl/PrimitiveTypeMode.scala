...
/** Internal field mapper instance used by Squeryl's own code (e.g. in [[org.squeryl.internals.DatabaseAdapter]]). */
private[squeryl] object InternalFieldMapper extends PrimitiveTypeMode

/**
 * The default type mode that makes all built-in Squeryl types available for use in queries
 * and schema definitions.
 *
 * This trait mixes in [[org.squeryl.dsl.QueryDsl]] (providing the query DSL) and [[org.squeryl.internals.FieldMapper]]
 * (providing the type registry), then re-exports every typed expression factory from
 * [[org.squeryl.internals.FieldMapper.PrimitiveTypeSupport]] as an implicit val. It also provides
 * implicit conversions from Scala values to [[org.squeryl.dsl.TypedExpression]] instances, enabling
 * literal values and `Query` results to participate in Squeryl's type-safe expression system.
 *
 * Applications should create a single object extending this trait and import its members:
 * {{{
 ...
 * ==java.time Support==
 *
 * The following `java.time` types are supported as first-class column types via their
 * implicit TEFs:
 *  - `LocalDate` (via [[localDateTEF]]) -- maps to SQL `DATE`
 *  - `LocalTime` (via [[localTimeTEF]]) -- maps to SQL `TIME`
 *  - `LocalDateTime` (via [[localDateTimeTEF]]) -- maps to SQL `TIMESTAMP`
 *  - `OffsetTime` (via [[offsetTimeTEF]]) -- maps to SQL `TIME`
 *  - `OffsetDateTime` (via [[offsetDateTimeTEF]]) -- maps to SQL `TIMESTAMP`
 *  - `Instant` (via [[instantTEF]]) -- stored as `OffsetDateTime` at UTC offset; see
 *    [[org.squeryl.internals.FieldMapper.PrimitiveTypeSupport.instantTEF]] for details
 *
 * ==ZonedDateTime Conversions==
 *
 * `ZonedDateTime` is ''not'' a supported column type, but convenience implicit conversions
 * ([[zonedDateTimeToInstantTE]] and [[optionZonedDateTimeToInstantTE]]) allow passing
 * `ZonedDateTime` values directly into queries. These conversions are '''lossy''': the
 * time zone information is discarded and only the UTC instant is preserved.
 */
trait PrimitiveTypeMode extends QueryDsl with FieldMapper {
  ...
  /** Typed expression factory for `java.time.LocalDate`, mapped to SQL `DATE` via JDBC 4.2 `getObject`/`setObject`. */
  ...
  /** Typed expression factory for `java.time.LocalTime`, mapped to SQL `TIME` via JDBC 4.2 `getObject`/`setObject`. */
  ...
  /** Typed expression factory for `java.time.LocalDateTime`, mapped to SQL `TIMESTAMP` via JDBC 4.2 `getObject`/`setObject`. */
  ...
  /** Typed expression factory for `java.time.OffsetTime`, mapped to SQL `TIME` via JDBC 4.2 `getObject`/`setObject`. */
  ...
  /**
   * Typed expression factory for `java.time.Instant`. This is a ''non-primitive'' mapper: `Instant`
   * values are stored as `OffsetDateTime` with a UTC offset. See
   * [[org.squeryl.internals.FieldMapper.PrimitiveTypeSupport.instantTEF]] for conversion details.
   */
  ...
  /** Typed expression factory for `java.time.OffsetDateTime`, mapped to SQL `TIMESTAMP` via JDBC 4.2 `getObject`/`setObject`. */
  ...
  /**
   * Implicitly converts a `ZonedDateTime` to an `Instant` typed expression.
   *
   * '''Warning:''' this conversion is '''lossy'''. The time zone information (zone ID and its rules)
   * is discarded; only the underlying UTC instant is preserved. The resulting value is stored as an
   * `OffsetDateTime` at UTC, just like any other `Instant`.
   */
  implicit def zonedDateTimeToInstantTE(s: ZonedDateTime): TypedExpression[Instant, TInstant] =
    instantTEF.create(s.toInstant)

  /**
   * Implicitly converts an `Option[ZonedDateTime]` (wrapped as `Some`) to an `Option[Instant]`
   * typed expression.
   *
   * '''Warning:''' this conversion is '''lossy'''. The time zone information (zone ID and its rules)
   * is discarded; only the underlying UTC instant is preserved. The resulting value is stored as an
