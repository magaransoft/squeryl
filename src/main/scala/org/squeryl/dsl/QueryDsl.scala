...
/** Provides base implicit definitions for the query DSL, including a default no-op [[OptionalKeyedEntityDef]]. */
trait BaseQueryDsl {
...
/**
 * The main DSL trait that provides all query construction methods: `from`, `where`, `select`,
 * `join`, `groupBy`, `orderBy`, `compute`, `transaction`, `inTransaction`, aggregate functions
 * (`count`, `sum`, `avg`, `min`, `max`, etc.), relation builders (`oneToManyRelation`,
 * `manyToManyRelation`), composite key constructors, and implicit conversions for type-safe
 * query composition.
 *
 * Mixed into concrete type modes (e.g. [[org.squeryl.PrimitiveTypeMode]]) along with
 * [[org.squeryl.internals.FieldMapper]] to form the complete Squeryl API surface.
 */
trait QueryDsl
...
//      queryToIterable(view2QueryAll(t))
  ...
  /** Executes the block `a` within the given session, restoring any previously bound session afterward. */
  def using[A](session: AbstractSession)(a: => A): A =
    session.using(() => a)

  /** Executes the block within a new transaction obtained from the given session factory. */
  def transaction[A](sf: SessionFactory)(a: => A) =
    sf.newSession.withinTransaction(() => a)

  /** Executes the block within a transaction from the given session factory, reusing an existing transaction if one is active. */
  ...
  /** Executes the block within a new transaction using the given session. */
  ...
  /** Introduces a WHERE clause with the given logical condition. */
  def where(b: => LogicalBoolean): WhereState[Conditioned] =
    new fsm.QueryElementsImpl[Conditioned](Some(() => b), Nil)

  /** Introduces a WITH clause (Common Table Expressions) with the given queries. */
  def withCte(queries: Query[_]*): WithState =
    new fsm.WithState(queries.toList.map(_.copy(false, Nil)))

  /** Dereferences a typed expression, yielding its value for use in compute/aggregate contexts. */
  ...
  /** SQL `STDDEV_POP` aggregate function: population standard deviation. */
  ...
  /** SQL `MIN` aggregate function: returns the minimum value. */
  ...
  /** SQL `AVG` aggregate function: returns the average value. */
  def avg[T2 >: TOptionFloat, T1 <: T2, A1, A2](b: TypedExpression[A1, T1])(implicit
    f: TypedExpressionFactory[A2, T2]
  ) = f.convert(new FunctionNode("avg", Seq(b)))

  /** SQL `SUM` aggregate function: returns the sum of all values. */
  ...
  /** SQL `COALESCE`/`NVL` function: returns the first non-null argument. */
  ...
  /** SQL `NOT` operator: negates the given logical boolean expression. */
  ...
  /** SQL `LOWER` function: converts a string expression to lowercase. */
  ...
  /** SQL `EXISTS` predicate: returns true if the subquery returns any rows. */
  def exists[A1](query: Query[A1]) = new ExistsExpression(query.copy(false, Nil).ast, "exists")

  /** SQL `NOT EXISTS` predicate: returns true if the subquery returns no rows. */
  ...
  /** SQL `COUNT(expr, ...)` aggregate function. */
  def count(e: TypedExpression[_, _]*) = new CountFunction(e, false)

  /** SQL `COUNT(DISTINCT expr, ...)` aggregate function. */
  ...
  /** Executes a partial update on the given table using a DSL-style update statement. */
  def update[A](t: Table[A])(s: A => UpdateStatement): Discardable[Int] = t.update(s)

  /** Creates a many-to-many relation builder between the left and right tables. */
  ...
  /** Creates a many-to-many relation builder with a custom name for the association table. */
  ...
  /** Creates a one-to-many relation builder between the "one" and "many" tables. */
