...
/**
 * Base trait for database-specific SQL generation and JDBC interaction.
 *
 * A `DatabaseAdapter` is responsible for translating Squeryl's internal AST representations
 * ([[org.squeryl.dsl.ast.QueryExpressionElements]], [[org.squeryl.dsl.ast.ExpressionNode]], etc.)
 * into vendor-specific SQL strings, and for executing those statements against JDBC connections.
 * It also defines the SQL type declarations for all supported Scala types (e.g. `intTypeDeclaration`,
 * `timestampTypeDeclaration`, `localDateTypeDeclaration`).
 *
 * Concrete implementations exist for specific databases (e.g. PostgreSQL, H2, MySQL) and override
 * methods where vendor SQL dialects differ from the defaults defined here.
 *
 * ==java.time Type Declarations==
 *
 * The following methods define SQL column types for `java.time` types:
 *  - [[localDateTypeDeclaration]] -- `DATE`
 *  - [[localTimeTypeDeclaration]] -- `TIME`
 *  - [[localDateTimeTypeDeclaration]] -- `TIMESTAMP`
 *  - [[offsetTimeTypeDeclaration]] -- `TIME`
 *  - [[offsetDateTimeTypeDeclaration]] -- `TIMESTAMP`
 *
 * Note: there is intentionally '''no''' type declaration method for `java.time.Instant`. `Instant` is
 * a non-primitive type that is stored via `OffsetDateTime` (using a
 * [[org.squeryl.dsl.NonPrimitiveJdbcMapper]]), so it reuses [[offsetDateTimeTypeDeclaration]] for its
 * column DDL.
 */
trait DatabaseAdapter {

  /** Wrapper that pairs an element with positional flags for first/last iteration detection. */
  class Zip[T](val element: T, val isLast: Boolean, val isFirst: Boolean)

  /** Iterable wrapper that yields [[Zip]] elements with first/last position awareness. */
  ...
  /**
   * Writes a complete SQL SELECT statement for the given query expression into the statement writer.
   * This is the main entry point; it delegates to the protected four-argument overload with default
   * values (no inverse ordering, no TOP hint).
   *
   * @param qen the query expression elements (select list, from, where, group by, order by, etc.)
   * @param sw the statement writer that accumulates the SQL text and bind parameters
   */
  ...
  /**
   * Core SQL SELECT generation method. Writes the full query including CTEs (`WITH`), `SELECT`,
   * `FROM` (with join support), `WHERE`, `GROUP BY`, `HAVING`, `ORDER BY`, pagination (`LIMIT`/`OFFSET`),
   * and `FOR UPDATE` clauses.
   *
   * @param qen the query expression elements forming the AST
   * @param sw the statement writer accumulating SQL output
   * @param inverseOrderBy when `true`, all `ORDER BY` expressions are inverted (used for certain
   *                       pagination strategies that reverse the result set)
   * @param topHint an optional vendor-specific hint inserted after `SELECT` (e.g. SQL Server's `TOP n`)
   */
  ...
  /** SQL type declaration for `java.time.LocalDate` columns. Defaults to `"date"`. */
  def localDateTypeDeclaration = "date"

  /** SQL type declaration for `java.time.LocalTime` columns. Defaults to `"time"`. */
  ...
  /**
   * SQL type declaration for `java.time.LocalDateTime` columns. Defaults to `"timestamp"`.
   * Also used as the underlying column type for `java.time.Instant` (which is stored as
   * `OffsetDateTime` at UTC offset via [[org.squeryl.dsl.NonPrimitiveJdbcMapper]]).
   */
  def localDateTimeTypeDeclaration = "timestamp"

  /** SQL type declaration for `java.time.OffsetTime` columns. Defaults to `"time"`. */
  def offsetTimeTypeDeclaration = "time"

  /**
   * SQL type declaration for `java.time.OffsetDateTime` columns. Defaults to `"timestamp"`.
   * `java.time.Instant` has no dedicated type declaration method because it is stored as
   * `OffsetDateTime` (with UTC offset) and reuses this declaration.
   */
   ...
   */
  /**
   * Determines the SQL column type for a given field. The resolution order is:
   *  1. Explicit `dbType` annotation on the field
   *  2. Schema-level column type override via `columnTypeFor`
   *  3. Automatic mapping based on the field's native JDBC type (with special handling for
   *     `String` and `BigDecimal` which include length/precision)
   *
   * @param fmd the field metadata
  ...
  /**
   * Generates the SQL fragment for a single column in a `CREATE TABLE` statement, including the
   * column name, data type, default value, primary key constraint, nullability, and auto-increment
   * modifier.
   *
   * @param fmd the field metadata describing the column
   * @param isPrimaryKey whether this column is declared as the primary key
   * @param schema the schema, used for type overrides
   * @return the SQL column declaration string (e.g. `"  my_col varchar(128) not null"`)
   */
  ...
  /**
   * Writes a complete `CREATE TABLE` statement for the given table into the statement writer.
   *
   * @param t the table to create
   * @param sw the statement writer accumulating the DDL output
   * @param schema the schema, used for column type overrides
   */
  ...
  /**
   * Executes a SELECT query against the database, returning the JDBC `ResultSet` and the
   * `PreparedStatement` (which the caller must close after consuming the results).
   *
   * @param s the current database session
   * @param sw the statement writer containing the SQL and bind parameters
   * @return a tuple of `(ResultSet, PreparedStatement)`
   */
  ...
  /**
   * Writes a SQL `INSERT INTO ... VALUES (...)` statement for a single object.
   * Auto-incremented and non-insertable fields are excluded from the column list.
   *
   * @param o the object to insert
   * @param t the target table
   * @param sw the statement writer accumulating the SQL and bind parameters
   */
...
//    }
  ...
  /**
   * Writes a SQL `UPDATE ... SET ... WHERE pk = ?` statement for a single keyed entity.
   * Handles composite primary keys, optimistic concurrency counters (incrementing the counter
   * column and optionally adding it to the WHERE clause), and skips ID and non-updatable fields.
   *
   * @param o the object with updated field values
   * @param t the target table
   * @param sw the statement writer accumulating the SQL and bind parameters
   * @param checkOCC when `true`, adds the optimistic concurrency counter to the WHERE clause
   *                 so the update fails if the row was modified concurrently
   */
  ...
  /**
   * Writes a SQL `DELETE FROM ... WHERE ...` statement.
   *
   * @param t the target table
   * @param whereClause optional filter expression; if `None`, all rows are deleted
   * @param sw the statement writer accumulating the SQL and bind parameters
   */
...
//      org.squeryl.internals.Utils.throwError("Oracle Bust : " + a)
  ...
  /**
   * Maps a Scala/Java class to its SQL type declaration by probing the field mapper's sample value
   * and matching against known types via `instanceof` checks. This is the fallback path used by
   * [[databaseTypeFor(FieldMetaData)]] when no explicit or schema-level override is specified.
   *
   * The matching order matters: for example, `java.sql.Timestamp` is checked before `java.util.Date`
   * because `Timestamp extends Date`, and `LocalDateTime`/`OffsetTime`/`OffsetDateTime` are checked
   * before `LocalDate` for similar subtype safety. `Instant` does not appear here because its sample
   * value is an `OffsetDateTime` (the JDBC-level type), so it matches [[offsetDateTimeTypeDeclaration]].
   *
   * @param fieldMapper the field mapper to obtain a sample value from
