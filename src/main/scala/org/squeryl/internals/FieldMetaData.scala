...
/**
 * Metadata for a single field (column) in a mapped Scala class. Holds the field's name,
 * types (Scala and JDBC), getter/setter references, column attributes (primary key, auto-increment,
 * unique, etc.), and provides methods for reading/writing field values on entity instances.
 */
class FieldMetaData(
...
/**
 * Factory for constructing [[FieldMetaData]] instances and POSO (Plain Old Scala Object) factories.
 * The default implementation uses reflection; custom implementations can support alternative
 * instantiation strategies (e.g. for frameworks that use bytecode generation).
 */
